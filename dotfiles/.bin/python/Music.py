...
class HandleFile:
    ...
    def load_file(cls, filename: str) -> list[str]:
        ...
        data = []
        try:
            with open(str(filename), mode='r', encoding='utf-8') as load_file:
...
def scandir(path: str, exts: Iterable[str]) -> list[str]:
    """Scans a path for files with extensions matching exts."""
    return [i for i in os.listdir(path) if '.' in i and i.split('.')[1] in exts]

def escape_terminal_characters(
    use_list: list[str], reverse: bool = False
) -> list[str]:
...
def get_music(do_escape: bool = True) -> list[str]:
    """Returns a list of files with the extensions wav and mp3. If do_escape is True, replace certain characters with equivalents."""
    # Get the filenames
...
def main() -> str | int:
    """Main program that does stuff based on command arguments."""
    # Get arguments to work off of
    args = list(sys.argv[1:])
    splitargs: list[str] = sum([list(arg.split('=')) for arg in args if '=' in arg], [])
    ...
    if '-getpos' in splitargs and toret == 'NOARGS':
        ...
        if '-printform' in args:
            toret = 'Now Playing '+escape_terminal_characters(filedata, True)[pos]
        else:
...
##                    '\t'+str("Vtmp='"+HOME+"/Music/'$(sed $n'!d' .bin/dep/music.txt)"),
                    '\t'+str(f'VCurPlaying="echo \\"$(python3 {mepath} -getpos=$n -printform)\\""'),
                    '\t'+str('VOmxRun="omxplayer -o $VSoundMode $VMusicFile >> /dev/null"'),
                    '\t'+str('echo $VCurPlaying >> .bin/dep/tmp.sh'),
                    '\t'+str('echo $VOmxRun >> .bin/dep/tmp.sh'), 'done']
    ...
    if '-getbad' in args and toret == 'NOARGS':
        ...
        tmp = [filedata[ftmp.index(piece)] for piece in ftmp if not piece in good]
